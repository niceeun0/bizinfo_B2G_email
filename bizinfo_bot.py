...
def fetch_bizinfo_notices():
    ...
    for attempt in range(3):
        try:
            print(f"⏳ [수집 시도 {attempt+1}/3] requests로 기업마당 API 호출 중...")
            ...
            if response.status_code == 200:
                ...
                print(f"⚠️ [API 응답 오류]: 상태 코드 {response.status_code}")
        except Exception as e:
            print(f"⚠️ [연결 에러] 시도 {attempt+1}회 실패: {str(e)}")
            time.sleep(3)
            
    print("❌ [최종 실패] API 데이터를 가져오지 못했습니다.")
    return []

def crawl_detail_page(url):
    if not url or url == "#":
        return "상세 링크 없음", "규모 정보 없음"
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # 🔍 모집규모, 사업규모, 선정규모 등 다양한 형태를 완벽히 잡아내도록 정규식 대폭 개선
            scale_match = re.search(r"(모집규모|사업규모|지원규모|선정규모|모집인원|지원대상)[:\s]*([^\n\.]{2,30}?(?:내외|개사|개소|업체|명|개 기업|개 과제))", full_text)
            if scale_match:
                scale_text = scale_match.group(0).strip()
            else:
                # 보조 패턴: 숫자와 '업체' 또는 '내외'가 들어간 구문 탐색
...
def analyze_and_build_html(items):
    ...
    <body style="font-family:'Malgun Gothic', sans-serif; color:#333;">
        <div style="max-width:1500px; margin:0 auto; padding:20px;">
            <h2 style="color:#004aad;">📊 B2G 지원사업 상세 분석 및 제안 리포트</h2>
