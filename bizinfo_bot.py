    ...
    try:
        ...
        with urllib.request.urlopen(req, context=ctx, timeout=30) as response:
            if response.status == 200:
                ...
                print(f"🎯 [수집 성공] 총 {len(items)}건의 공고를 정상적으로 불러왔습니다.")
                return items
            else:
...
def analyze_and_build_html(items):
    ...
    for idx, item in enumerate(items, 1):
        ...
        # 원클릭 제안 판정: 지정된 서류 중 2개 이상 포함되어 있거나 필수 증빙이 요구되는 경우
        is_one_click_eligible = len(found_docs) >= 2 or "제출" in full_text

        if is_one_click_eligible:
            one_click_html = '<span style="background:#e6f4ea; color:#137333; padding:6px 10px; border-radius:4px; font-size:12px; font-weight:bold; display:inline-block;">⚡ 원클릭 제안 가능</span>'
        else:
            one_click_html = '<span style="color:#666; font-size:11px;">공고문 확인</span>'
    ...
    <body style="font-family:'Malgun Gothic', sans-serif; color:#333;">
        <div style="max-width:1500px; margin:0 auto; padding:20px;">
            <h2 style="color:#004aad;">📊 B2G 지원사업 원클릭 제안 맞춤형 분석 리포트</h2>
            <p>총 {len(items)}건의 공고에서 제안 서류 요구 사항이 분석되었습니다.</p>
            <table style="width:100%; border-collapse:collapse; margin-top:15px;">
                <thead>
                    <tr style="background-color:#f8fafc; color:#444; font-size:12px;">
                        ...
                        <th style="padding:10px; border-bottom:2px solid #ddd;">원클릭 제안 판정</th>
...
def send_email(html_body):
    ...
    msg["Subject"] = "📊 [원클릭 제안 리포트] B2G 지원사업 맞춤형 서류 분석 현황"
