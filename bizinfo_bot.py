...
def fetch_bizinfo_notices():
    ...
    try:
        print("⏳ [수집 시작] 기업마당 API 서버 호출 중...")
        resp = requests.get(BIZINFO_API_URL, params=params, headers=headers, timeout=25, verify=False)
...
def send_email(html_body):
    ...
    receiver_email = os.environ.get("MAIL_RECEIVER") or sender_email
    ...
    try:
        ...
        server.quit()
        print("🎉 [이메일 전송 성공] 지정하신 메일함으로 리포트가 발송되었습니다!")
    except Exception as e:
