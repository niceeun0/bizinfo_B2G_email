...
DOC_KEYWORDS = [
    "사업신청서", "사업계획서", "동의서", "확약서", 
    "지방세", "납세증명서", "국세", "완납", 
    "사업자등록증명", "부가가치세", "재무제표", "신용평가", 
    "인감증명서", "법인등기", "주주명부", 
    "건강보험", "자격득실", "가입자명부", "4대사회보험", 
    "견적서", "소개서", "이력서", "통장사본", "정산"
]

def fetch_bizinfo_notices():
    # 넉넉하게 최근 50개를 가져온 뒤 날짜로 필터링합니다.
    ...
    try:
        ...
        with urllib.request.urlopen(req, context=ctx, timeout=30) as response:
            if response.status == 200:
                ...
                print(f"🎯 [수집 성공] 총 {len(items)}건의 전체 공고 중 신규 공고를 필터링합니다.")
                
                # 📅 어제 날짜 계산 (YYYYMMDD 형식)
                yesterday_str = (datetime.now() - timedelta(days=1)).strftime("%Y%m%d")
                print(f"📌 기준 일자 (어제): {yesterday_str}")
                
                filtered_items = []
                for item in items:
                    # 등록일자 필드 확인 (pblancDtg 또는 rceptBeginDe 등 API 제공 필드)
                    reg_date = item.get("pblancDtg") or item.get("regDate") or item.get("rceptBeginDe") or ""
                    reg_date_clean = re.sub(r'[^0-9]', '', reg_date)[:8] # 숫자만 추출하여 YYYYMMDD 형태로 맞춤
                    
                    # 날짜 정보가 정확히 일치하거나, 혹은 정보가 없으면 누락 방지를 위해 포함하되 기본적으로 어제자 우선 필터링
                    if reg_date_clean == yesterday_str:
                        filtered_items.append(item)
                
                # 만약 어제 올라온 공고가 딱 0개라면 테스트를 위해 가장 최근 공고 1~2개 정도를 보여줄 수도 있습니다.
                if not filtered_items:
                    print("ℹ️ 어제 등록된 신규 공고가 없어, 최신 공고 목록을 대상으로 진행합니다.")
                    filtered_items = items[:5] # 안전을 위해 최근 5개만 샘플링
                else:
                    print(f"✨ 어제 등록된 신규 공고 {len(filtered_items)}건을 필터링했습니다.")
...
def crawl_detail_page(url):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            scale_match = re.search(r"(모집규모|사업규모|지원규모|선정규모|모집인원)[:\s]*([^\n\.]{2,30}?(?:내외|개사|개소|업체|명|개 기업|개 과제))", full_text)
            if scale_match:
                scale_text = scale_match.group(0).strip()
            else:
                fallback_match = re.search(r"(\d+\s*(?:개사|개소|업체|내외|명))", full_text)
                scale_text = f"규모: {fallback_match.group(1)}" if fallback_match else "공고문 참조"
...
def analyze_and_build_html(items):
    ...
    <body style="font-family:'Malgun Gothic', sans-serif; color:#333;">
        <div style="max-width:1500px; margin:0 auto; padding:20px;">
            <h2 style="color:#004aad;">📊 B2G 지원사업 신규 공고 자동화 리포트 (전날 업로드 기준)</h2>
            <p>전날 등록된 신규 공고 총 {len(items)}건에 대한 분석 리포트입니다.</p>
...
def send_email(html_body):
    ...
    msg["Subject"] = "📊 [신규 공고 리포트] 전날 등록된 B2G 지원사업 현황"
