...
_COLUMN_WIDTH_OVERRIDES = {
    ...
    "지원규모": 32, "기타서류": 32, "비고": 35, "문의처": 22, "담당이메일": 24,
...
def main():
    ...
    def process_single_item(item):
        ...
        # 나이스평가정보 타겟 키워드 매칭 분석
        target_keywords = ["데이터", "ai", "ax", "공급", "제공기업", "공급기업", "참여기업", "참여 기업", "공급 기업", "기업db", "db", "기업데이터"]
        combined_text_for_match = f"{title} {body_text}".lower()
        matched_kws = [kw for kw in target_keywords if kw in combined_text_for_match]
        matched_kws_unique = list(dict.fromkeys(matched_kws))
        if matched_kws_unique:
            remarks = f"[나이스평가정보 타겟 매칭] 매칭 키워드: {', '.join(matched_kws_unique)}"
        else:
            remarks = "-"
        ...
        row = {
            ...
            "_matched_kws": matched_kws_unique,
        }
